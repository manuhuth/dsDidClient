...
ds.did <- function(data = NULL, yname = NULL, tname = NULL, idname = NULL, gname = NULL,
  ...
  for (g in g_periods) {
    for (t in t_periods) {
      ...
      # run logistic regression on P(G=g |X, G_g + C = 1) -> only treated in g or never treated (observations in df_analysis)
      ...
      ds.sendToServer(eta_treat,
        newobj = "eta_treat_server",
        datasources = datasources_subsetted
      )
      ...
      dsBaseClient::ds.asNumeric(
        x.name = "inf_treat_2", newobj = "inf_treat_2",
        datasources = datasources_subsetted
      )

      ds.sendToServer(mean_G,
        newobj = "mean_G",
        datasources = datasources_subsetted
      )
      dsBaseClient::ds.make("inf_treat_1 / mean_G",
        newobj = "inf_treat_1_G",
        datasources = datasources_subsetted
      )
      dsBaseClient::ds.make("inf_treat_2 / mean_G",
        newobj = "inf_treat_2_G",
        datasources = datasources_subsetted
      )
      ...
      ds.sendToServer(eta_cont,
        newobj = "eta_cont_server",
        datasources = datasources_subsetted
      )
      ...
      dsBaseClient::ds.asNumeric(
        x.name = "inf_cont_3", newobj = "inf_cont_3",
        datasources = datasources_subsetted
      )
      ...
      dsBaseClient::ds.asNumeric(
        x.name = "inf_cont_1", newobj = "inf_cont_1",
        datasources = datasources_subsetted
      )
      dsBaseClient::ds.asNumeric(
        x.name = "inf_cont_2", newobj = "inf_cont_2",
        datasources = datasources_subsetted
      )
      ...
      odds_mean <- dsBaseClient::ds.mean("odds",
        type = "combined",
        datasources = datasources_subsetted
      )$Global.Mean[1]

      ds.sendToServer(odds_mean,
        newobj = "odds_mean",
        datasources = datasources_subsetted
      )
      dsBaseClient::ds.make("inf_cont_helper / odds_mean",
        newobj = "inf_cont_helper_p",
        datasources = datasources_subsetted
      )

      dsBaseClient::ds.make("inf_cont_3 / odds_mean",
        newobj = "inf_cont_3_p",
        datasources = datasources_subsetted
      )
      ...
      ds.AppendInfluence(
        df = "influence_matrix", influences = "dr_att_inf_func_n",
      ...
      ds.AppendInfluence(
        df = "influence_matrix_not_divided",
      ...
      dsBaseClient::ds.make("dr_att_inf_func * dr_att_inf_func",
        newobj = "psi_inner_product",
        datasources = datasources_subsetted
      )

      se_dr_att <- (dsBaseClient::ds.mean("psi_inner_product",
        type = "combined",
        datasources = datasources_subsetted
      )$Global.Mean[1])^0.5 / sqrt(n)
  ...
  if (bstrap) {
    ...
    # cannot use original mboot function since this function requires the data to be in dp
    ...
    # Compute new critical value for uniform confidence band
    # See paper for details
    b_sigma <- apply(
      bres, 2,
      function(b) {
        (stats::quantile(b, .75, type = 1, na.rm = TRUE) -
          stats::quantile(b, .25, type = 1, na.rm = TRUE)) / (stats::qnorm(.75) - stats::qnorm(.25))
      }
    )
    ...
    if (cband) {
      # Compute sup-t confidence band
      bT <- apply(bres, 1, function(b) max(abs(b / b_sigma), na.rm = TRUE))
      z_value <- stats::quantile(bT, 1 - alpha, type = 1, na.rm = TRUE)
      if (z_value >= 7) {
        warning("Simultaneous critical value is arguably 'too large' to be reliable. This usually happens when the number of observations per group is small and/or there is little variation in outcomes.")
      }
    }
  ...
  if (length(pre_V) == 0) {
    ...
    W <- NULL
    W_pval <- NULL
  } else if (any(is.na(pre_V))) {
    warning("Pre-treatment values contain NAs; not returning pre-test Wald statistic.")
    W <- NULL
    W_pval <- NULL
  } else if (rcond(pre_V) <= .Machine$double.eps) {
    # Singular covariance matrix for pre-treatment periods
    warning("Singular covariance matrix detected; not returning pre-test Wald statistic.")
    W <- NULL
    W_pval <- NULL
  } else {
    W <- n_global * t(pre_att) %*% solve(pre_V) %*% pre_att
    q <- length(pre_att) # Number of restrictions
  ...
  return(did::MP(
    group = group, t = tt, att = att, V_analytical = V, se = se, c = z_value,
